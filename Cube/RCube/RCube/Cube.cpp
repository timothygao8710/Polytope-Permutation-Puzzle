	...
	for (int quadrant = QState; quadrant < 4; ++quadrant)
	{
		...
		for (uint r = start; r < R1; ++r)
		{
			...
			while (true)
			{
                ...
                // codicon: I commented this line and uncommented below otherwise moves are obfuscated
				// OptomizedMovement(mstack, stkptr, r, src, dst, sq, dq);

                // Codicon comment:
                // For some reason below is not equivalent to OptomizedMovement()
                // The moves differ:
                // For 33x33x33, this is better by 1.5k, for smaller the mean appears the same?
                //      The larger the cube, the bigger the difference.
				// Move the pieces
